...
static void
_range_remove_element(asn1cnst_range_t *range, int idx) {
...
static int
_range_insert(asn1cnst_range_t *into, asn1cnst_range_t *cr) {
    assert(!cr->elements);
...
static asn1cnst_range_t *
_range_clone(const asn1cnst_range_t *range) {
...
static int
_range_merge_in(asn1cnst_range_t *into, asn1cnst_range_t *cr) {
    asn1cnst_range_t *r;
    ...
    if(into->extensible) into->not_OER_visible = 1;
...
static enum range_fill_result
_range_fill(asn1p_value_t *val, const asn1cnst_range_t *minmax,
            asn1cnst_edge_t *edge, asn1cnst_range_t *range,
            enum asn1p_constraint_type_e type, int lineno) {
...
static int
_range_partial_compare(const void *pa, const void *pb) {
...
static void
_range_partial_sort_elements(asn1cnst_range_t *r) {
...
static int
_range_intersection(asn1cnst_range_t *range, const asn1cnst_range_t *with,
    ...
    if(is_oer) {
        ...
        if(with->extensible) range->not_OER_visible = 1;
...
_range_union(asn1cnst_range_t *range) {
    ...
    for(i = 1; i < range->el_count; i++) {
        ...
        if(_range_overlap(ra, rb)) {
            ...
            if(_edge_compare(&ra->right, &rb->right) > 0) rb->right = ra->right;
        } else {
            /*
...
asn1cnst_range_t *
asn1constraint_compute_OER_range(const char *dbg_name,
                                 asn1p_expr_type_e expr_type,
                                 const asn1p_constraint_t *ct,
                                 enum asn1p_constraint_type_e requested_ct_type,
                                 const asn1cnst_range_t *minmax, int *exmet,
                                 enum cpr_flags cpr_flags) {
...
asn1cnst_range_t *
asn1constraint_compute_PER_range(const char *dbg_name,
                                 asn1p_expr_type_e expr_type,
                                 const asn1p_constraint_t *ct,
                                 enum asn1p_constraint_type_e requested_ct_type,
                                 const asn1cnst_range_t *minmax, int *exmet,
    ...
    /* Due to pecularities of PER constraint handling, we don't enable strict
    ...
    if(mantissa->left.type == ARE_VALUE && mantissa->right.type == ARE_VALUE
       ...
       && exponent->left.value >= -149 && exponent->right.value <= 104) {
        ...
        if(mantissa->left.type == ARE_VALUE && mantissa->right.type == ARE_VALUE
           ...
           && exponent->right.type == ARE_VALUE && exponent->left.value >= -1074
           && exponent->right.value <= 971) {
...
asn1constraint_compute_constraint_range(
    ...
    if(asn1constraint_compatible(expr_type, requested_ct_type,
                                 cpr_flags & CPR_simulate_fbless_SIZE)
       != 1) {
        errno = EINVAL;
        return 0;
    }
    ...
    case ACT_CT_FROM:
        ...
        if(tmp) {
            ...
            if(errno == ERANGE) {
                ...
                range->extensible = 1;
                if(range->extensible) range->not_OER_visible = 1;
                tmp = range;
            } else {
                _range_free(range);
            }
    ...
    case ACT_CA_INT: /* SIZE(1..2) ^ FROM("ABCD") */
        ...
        for(i = 0; i < ct->el_count; i++) {
            ...
            ret = _range_intersection(range, tmp, ct->type == ACT_CA_SET,
                                      cpr_flags & CPR_strict_OER_visibility);
            _range_free(tmp);
            if(ret) {
                _range_free(range);
    ...
    case ACT_CA_UNI: /* SIZE(1..2) | FROM("ABCD") */
        ...
        for(i = 0; i < ct->el_count; i++) {
            tmp = asn1constraint_compute_constraint_range(
                dbg_name, expr_type, ct->elements[i], requested_ct_type, minmax,
                exmet, cpr_flags);
            if(!tmp) {
                if(errno == ERANGE) {
                    range->extensible = 1;
        ...
        if(range->not_PER_visible && (cpr_flags & CPR_strict_PER_visibility)) {
            /*
             * X.691, #9.3.19:
