...
asn1f_check_class_object(arg_t *arg) {
    ...
    if(!eclass->with_syntax) {
        DEBUG("Can't process classes without %s just yet", "WITH SYNTAX");
        return 0;
    }
...
asn1f_parse_class_object(arg_t *arg) {
    ...
    struct parse_object_key key = {
        .arg = arg, .expr = expr, .eclass = eclass, .sequence = 0};
...
_asn1f_parse_class_object_data(arg_t *arg, asn1p_expr_t *eclass,
                               ...
                               asn1p_wsyntx_t *syntax, const uint8_t *buf,
                               const uint8_t *bend, int optional_mode,
                               const uint8_t **newpos, int counter) {
    ...
    TQ_FOR(chunk, (&syntax->chunks), next) {
        ...
        case WC_FIELD: {
            ...
            if(cell == NULL) {
                FATAL(
                    "Field reference %s found in WITH SYNAX {} clause does not "
                    "match actual field in Object Class %s",
                    chunk->content.token, eclass->Identifier, eclass->_lineno);
                if(newpos) *newpos = buf;
                return -1;
            }
...
asn1f_next_literal_chunk(asn1p_wsyntx_t *syntax, asn1p_wsyntx_chunk_t *chunk,
                         const uint8_t *buf) {
