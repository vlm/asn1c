...
asn1f_fix_module__phase_1(arg_t *arg) {
    ...
    TQ_FOR(omod, &arg->asn->modules, mod_next) {
        ...
        if(omod->module_oid && arg->mod->module_oid) {
            ...
            if(asn1p_oid_compare(omod->module_oid, arg->mod->module_oid) == 0) {
                FATAL(
                    "ASN.1 module %s in %s "
