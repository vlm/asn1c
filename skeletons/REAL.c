...
#define SRV_SET(foo, val) \
    { foo, sizeof(foo) - 1, val }
...
ssize_t
REAL__dump(double d, int canonical, asn_app_consume_bytes_f *cb,
           void *app_key) {
    char local_buf[64];
    char *buf = local_buf;
...
asn_REAL2double(const REAL_t *st, double *dbl_value) {
    ...
    case 0x00: { /* X.690: 8.5.7 */
        ...
        if(st->buf[st->size] != '\0' || memchr(st->buf, ',', st->size)) {
            ...
            for(p = st->buf + 1, end = st->buf + st->size; p < end; b++, p++)
                *b = (*p == ',') ? '.' : *p;
            *b = '\0';
        } else {
...
int
CC_ATTR_NO_SANITIZE("float-cast-overflow")
