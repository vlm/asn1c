...
asn1c_make_identifier(enum ami_flags_e flags, asn1p_expr_t *expr, ...) {
    ...
    if(expr) {
        ...
        if(expr->spec_index != -1) {
            static char buf[32];
            size += 1
                    + snprintf(buf, sizeof buf, "%dP%d", expr->_lineno,
                               expr->spec_index);
...
asn1c_type_name(arg_t *arg, asn1p_expr_t *expr, enum tnfmt _format) {
    ...
    if(0) DEBUG("asn1c_type_name(%s: 0x%x)", expr->Identifier, expr->expr_type);

    switch(expr->expr_type) {
    case A1TC_REFERENCE:
        typename =
            expr->reference->components[expr->reference->comp_count - 1].name;
        if(typename[0] == '&') {
            arg_t tmp = *arg;
    ...
    case TNF_UNMODIFIED:
        return asn1c_make_identifier(
            AMI_MASK_ONLY_SPACES | AMI_NODELIMITER, 0, MODULE_NAME_OF(exprid),
