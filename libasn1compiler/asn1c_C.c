...
asn1c_lang_C_type_SEQUENCE_def(arg_t *arg,
    ...
    if(expr_elements_count(arg, expr)) {
        ...
        if((roms_count + aoms_count)
           && (arg->flags & (A1C_GEN_OER | A1C_GEN_PER))) {
            int elm = 0;
            int comma = 0;
            comp_mode = 0;
...
asn1c_lang_C_type_SIMPLE_TYPE(arg_t *arg) {
    ...
    if(arg->embed) {
        ...
        OUT("%s%s_t", (expr->marker.flags & EM_INDIRECT) ? "*" : " ",
            MKID(expr));
    }
...
emit_member_table(arg_t *arg, asn1p_expr_t *expr,
    ...
    if(expr->constraints) {
        if(arg->flags & A1C_NO_CONSTRAINTS) {
            ...
            OUT(" memb_%s_constraint_%d", id, arg->expr->_type_unique_index);
        }
    } else {
        OUT("0");
    }
...
emit_include_dependencies(arg_t *arg) {
    ...
    TQ_FOR(memb, &(expr->members), next) { expr_break_recursion(arg, memb); }
        ...
        if((!(memb->expr_type & ASN_CONSTR_MASK)
           ...
           || memb->meta_type == AMT_TYPEREF) {
            if(memb->marker.flags & EM_UNRECURSE) {
                GEN_POSTINCLUDE(asn1c_type_name(arg, memb, TNF_INCLUDE));
            } else {
                GEN_INCLUDE(asn1c_type_name(arg, memb, TNF_INCLUDE));
            }
        }
    }
