...
static asn1_hash_t *used_names_hash;
...
c_name_clash_finder_init() {
    asn1_hash_destroy_with_cleanup(used_names_hash, cleanup);
    used_names_hash = (asn1_hash_t *)NULL;
}
...
register_global_name(asn1p_expr_t *expr, const char *name) {
    ...
    n = asn1_hash_search(used_names_hash, name);
    if(n && !(expr->_mark & TM_NAMEGIVEN) && (expr != n->expr)) {
        n->clashes_with = expr;
        expr->ref_cnt++;
        return;
    }
    ...
    expr->ref_cnt++;
    n->name = strdup(name);
    asn1_hash_insert(&used_names_hash, name, n);
}

struct global_data {
    arg_t *arg;
    size_t n_clashes;
    const size_t max_clashes;
};

static void
clash_found(void *fn_data, void *node_data) {
    struct global_data *d = (struct global_data *)fn_data;
    struct intl_name *n = (struct intl_name *)node_data;
    arg_t *arg = d->arg;

    if(!n->clashes_with) return;
    if(d->n_clashes++ > d->max_clashes) return;

    FATAL("Name \"%s\" is generated by %s.%s at line %s:%d and "
          "%s.%s at line %s:%d",
...
c_name_clash(arg_t *arg) {
    struct global_data d = {
        .arg = arg,
        .n_clashes = 0,
        .max_clashes = 5,
    };

    asn1_hash_traverse_fn_with_data(used_names_hash,
                                    clash_found, &d);

    if(d.n_clashes > d.max_clashes) {
        FATAL("... %zu more name clashes not shown", d.n_clashes - d.max_clashes);
    }

    return d.n_clashes > 0;
}
