...
static int
ulong_optimization(arg_t *arg, asn1p_expr_type_e etype,
                   asn1cnst_range_t *r_size, asn1cnst_range_t *r_value) {
...
asn1c_emit_constraint_checking_code(arg_t *arg) {
    ...
    case ASN_BASIC_ENUMERATED:
        if(asn1c_type_fits_long(arg, arg->expr) == FL_NOTFIT) produce_st = 1;
        break;
    case ASN_BASIC_REAL:
...
asn1c_emit_constraint_tables(arg_t *arg, int got_size) {
    ...
    if(use_table) {
        ...
        if((arg->flags & A1C_GEN_PER) && (etype & ASN_STRING_KM_MASK)) {
            int c;
        ...
        if(asn1c_type_fits_long(arg, arg->expr) == FL_FITS_UNSIGN) {
            ...
            if(r_value->el_count == 0
               && (
                   ...
                   (r_value->left.type == ARE_VALUE && r_value->left.value == 0
